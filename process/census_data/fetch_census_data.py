...
class FetchException(Exception):
    ...
    def __init__(self, response: httpx.Response):
        super().__init__(f"{response.status_code} retrieving {response.url}: {response.text}")


def fetch_county_tracts(year: int, state: str, county: str, variables: list[str]) -> pd.DataFrame:
    """
...
def build_census_csv(year: int, output_filename: Path) -> pd.DataFrame:
    ...
    for name, fips in COUNTIES.items():
        print(f"Fetching {name.title()} County (state = {STATE_FIPS}, county = {fips}) tracts...")
        df_part = fetch_county_tracts(year = year, state = STATE_FIPS, county = fips, variables = VARIABLES)
        dfs.append(df_part)
